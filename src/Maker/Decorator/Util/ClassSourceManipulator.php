...
final class ClassSourceManipulator
{
    ...
    public function addClassProperty(
        ...
        if (is_null($typeHint)) {
            $typeHint = $this->addUseStatementIfNecessary($mapping->type);
        }
    ...
    public function addConstructorPropertyPromotion(
        string $name,
        $defaultValue = self::DEFAULT_VALUE_NONE,
        array $attributes = [],
        array $comments = [],
        ...
        $this->addNodeAfterConstructorProperties($newPropertyNode);
    }
    ...
    private function buildParam(
        ...
        $newParamBuilder = (new Builder\Param($name))->makePrivate();

        if (null !== $propertyType) {
            $newParamBuilder->setType($propertyType);
        }

        if ($comments) {
            // setDocComment not available in nikic/php-parser
            // $newParamBuilder->setDocComment($this->createDocBlock($comments));
        }

        if (self::DEFAULT_VALUE_NONE !== $defaultValue) {
            $newParamBuilder->setDefault($defaultValue);
        }

        return $newParamBuilder;
    }

    private function addNodeAfterConstructorProperties($propertyNode): void
    {
        // Comment faire un statement pour ajouter une propriété au constructeur ?
        // $stmt = ??;
        // $this->addStatementToConstructor(
        //     new Node\Stmt\Expression(new Node\Expr\Assign(
        //         new Node\Expr\PropertyFetch(new Node\Expr\Variable('this'), $propertyName),
        ...
        // $constructorNode->stmts[] = $propertyNode;
        $constructorNode->params[] = $propertyNode;
        $this->updateSourceCodeFromNewStmts();
    }
    ...
    public function buildAttributeNode(string $attributeClass, array $options, ?string $attributePrefix = null): Node\Attribute
    {
        ...
        $class = $attributePrefix ?
            \sprintf('%s\\%s', $attributePrefix, Str::getShortClassName($attributeClass)) :
