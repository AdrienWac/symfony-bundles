...
final class MakerTestEnvironment
{
    ...
    private const MAKER_REPO_NAME = 'maker-repo';
    ...
    private function __construct(
        ...
        $this->path = $this->cachePath .
            \DIRECTORY_SEPARATOR .
            $testDetails->getUniqueCacheDirectoryName() .
            '_' .
            $directoryName;

        $this->makerRepoPath = sprintf('%s/%s', $this->cachePath, self::MAKER_REPO_NAME);
    }
    ...
    {
        // Copy HexagonalMakerBundle to a "repo" directory (maker-repo) for tests
        if (!file_exists($makerRepoPath = \sprintf('%s/maker-repo', $this->cachePath))) {
            MakerTestProcess::create(
                \sprintf('git clone %s %s', $this->rootPath, $makerRepoPath),
                $this->cachePath
            )->run();
        }

        // Création d'un projet pour simuler un projet symfony et l'installation du bundle courant
        if (!$this->fs->exists($this->flexPath)) {
            $this->buildFlexSkeleton();
        }

        // Création d'un projet concret depuis le skeleton
        if (!$this->fs->exists($this->path)) {
            try {
                ...
                // Ici j'essaye d'installer le vendor adrienlbt/hexagonal-maker-bundle
                $this->composerRequireHexagonalMakerBundle($this->path);

                // install any missing dependencies
                $dependencies = $this->determineMissingDependencies();
                if ($dependencies) {
                    // -v actually silences the "very" verbose output in case of an error
                    $composerProcess = MakerTestProcess::create(
                        \sprintf(
                            'composer require %s -v',
                    ...
                    if (!$composerProcess->isSuccessful()) {
                        throw new \Exception(
                            \sprintf(
                                'Error running command: composer require %s -v. Output: "%s". Error: "%s"',
                                implode(' ', $dependencies),
                                $composerProcess->getOutput(),
                                $composerProcess->getErrorOutput()
                            )
                        );
                    }
                }
                // Dump autoload
                $composerProcess = MakerTestProcess::create('composer dump-autoload', $this->path)
                    ->run(true);
                ...
                MakerTestProcess::create(
                    \sprintf('git diff --quiet || ( %s && git add . && git commit -a -m "second commit" )', self::GIT_CONFIG),
                    $this->path
                )->run();
            } catch (\Exception $e) {
    ...
    public function runTwigCSLint(string $file): MakerTestProcess
    {
        ...
        return MakerTestProcess::create(
            \sprintf('php tools/twigcs/vendor/bin/twigcs --config ./tools/twigcs/.twig_cs.dist %s', $this->path . '/' . $file),
            $this->rootPath
        )->run(true);
    }
    ...
    private function buildFlexSkeleton(): void
    {
        ...
        if (!$this->isWindows) {
            // $this->composerRequireHexagonalMakerBundle(\sprintf('%s/%s', $this->cachePath, $flexProjectDir));
        ...
        MakerTestProcess::create(
            'composer require phpunit browser-kit symfony/css-selector --prefer-dist --no-progress --no-suggest',
            $this->flexPath
        )->run();

        MakerTestProcess::create('composer dump-autoload', $this->flexPath)
                        ->run();
        ...
        MakerTestProcess::create(
            \sprintf('git init && %s && git add . && git add vendor/ -f && git commit -a -m "first commit"', self::GIT_CONFIG),
            $this->flexPath
        )->run();
    }
    ...
    public function createInteractiveCommandProcess(
        string $commandName,
        array $userInputs,
...
echo json_encode($missingDependencies);
    ...
    private function composerRequireHexagonalMakerBundle(string $projectDirectory): void
    {
        MakerTestProcess::create('composer require --dev adrienlbt/hexagonal-maker-bundle', $projectDirectory)
            ->run()
        ;

        $makerRepoSrcPath = \sprintf('%s/src', $this->makerRepoPath);
    ...
    /**
     * Creates a directory in skeleton project to test the hexagonal maker bundle.
     */
    private function createHexagonalMakerDirectory(string $projectPath): void
    {
        $this->fs->mkdir($projectPath . '/src/Domain');
        $this->fs->mkdir($projectPath . '/src/Domain/Entity');
        $this->fs->touch($projectPath . '/src/Domain/Entity/.gitignore');
        $this->fs->mkdir($projectPath . '/src/Application');
        $this->fs->touch($projectPath . '/src/Application/.gitignore');
        $this->fs->mkdir($projectPath . '/src/Infrastructure');
        $this->fs->touch($projectPath . '/src/Infrastructure/.gitignore');
    }

    /**
     * Add path of the folder for hexagonal namespace in the composer.json file.
     *
     * @param string $composerJsonPath
     * @return void
     */
    private function createHexagonalMakerNameSpace(string $composerJsonPath): void
    {
        ...
        $composerJson['autoload']['psr-4']['Domain\\'] = 'src/Domain/';
        $composerJson['autoload']['psr-4']['Application\\'] = 'src/Application/';
        $composerJson['autoload']['psr-4']['Infrastructure\\'] = 'src/Infrastructure/';
    ...
    /**
     * Ajoute le chemin du dossier test/tmp/cache/maker-repo en tant que repositorie
     * pour le  vendor adrienlbt/hexagonal-maker-bundle dans le composer json
     * du skeleton.
     *
     * @param string $composerJsonPath
     * @return void
     */
    private function addHexagonalMakerBundleRepositorieToSkeletonComposerFile(string $composerJsonPath): void
    {
        $composerJson = json_decode(
            file_get_contents($composerJsonPath),
            true,
        ...
        $composerJson['repositories']['adrienlbt/hexagonal-maker-bundle'] = [
            'type' => 'path',
            'url' => \sprintf('%s%smaker-repo', $this->cachePath, \DIRECTORY_SEPARATOR),
            'options' => [
                'versions' => [
                    'adrienlbt/hexagonal-maker-bundle' => '9999.99', // Arbitrary version to avoid stability conflicts
                ],
            ],
        ];
